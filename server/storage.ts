...
export const storageHealthy = () =>
  s3.send(new HeadBucketCommand({ Bucket: bucket }), { abortSignal: AbortSignal.timeout(3000) });
export async function objectStream(tusId: string) {
  ...
  const result = await s3.send(new GetObjectCommand({ Bucket: bucket, Key: tusId.split('+')[0] }), {
