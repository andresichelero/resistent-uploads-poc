...
app.addHook('onRequest', async (req, reply) => {
  ...
  if (!/^(localhost|127\.0\.0\.1)(:\d+)?$/.test(host))
    return reply.code(403).send({ error: 'This demo is available on localhost only.' });
...
app.get('/api/health', async (_, reply) => {
  try {
    ...
    const response = await fetch(`${tusUrl}/files/`, {
      method: 'OPTIONS',
    ...
    return reply
      .code(503)
      .send({ status: 'starting', scanRequired, maxSize: MAX_SIZE, retentionHours: 24 });
  }
});
...
app.post('/api/uploads', async (req, reply) => {
  ...
  if (
    ...
    return reply.code(400).send({ error: 'A UUID v4 Idempotency-Key is required.' });
  ...
  if (
    ...
    return reply.code(409).send({ error: 'This request ID belongs to another file.' });
  if (!existing)
    db.put({
      ...input,
      id,
      ...
      integrity: 'pending',
      actualSha256: null,
      scan: scanRequired ? 'pending' : 'not_run',
      scannerVersion: null,
      detail: null,
      deleting: false,
    });
  ...
  if (!db.get(id)?.tusId && !creating.has(id)) {
    ...
    const creation = (async () => {
      const response = await fetch(`${tusUrl}/files/`, {
        ...
        headers: {
          ...
          'Upload-Metadata': `recordId ${Buffer.from(id).toString('base64')}`,
        },
        signal: AbortSignal.timeout(30000),
      });
      if (response.status !== 201)
        throw new Error('Upload service could not create the transfer. Retry with the same file.');
      const location = response.headers.get('location');
      if (!location) throw new Error('Upload service returned no location.');
    ...
    void creation
      .finally(() => {
        creating.delete(id);
        service.creating.delete(id);
      })
      .catch(() => undefined);
  }
  try {
    await creating.get(id);
    return reply.code(201).send(service.public(db.get(id)!));
  } catch (error) {
...
app.get<{ Params: { id: string } }>('/api/uploads/:id', (req, reply) => {
  ...
  return r ? service.public(r) : reply.code(404).send({ error: 'Upload not found.' });
});
app.delete<{ Params: { id: string } }>('/api/uploads/:id', async (req, reply) => {
  try {
    ...
    return reply
      .code(503)
      .send({ error: 'Deletion pending. Storage will be retried automatically.' });
  }
});
app.post<{ Params: { id: string } }>('/api/uploads/:id/verify', async (req, reply) => {
  const r = db.get(req.params.id);
  if (!r) return reply.code(404).send();
  if (r.offset !== r.size || !r.tusId || r.deleting)
    return reply.code(409).send({ error: 'Finish the upload before verification.' });
  // Block download before scheduling async work, including during re-verification.
  db.patch(r.id, { integrity: 'pending' });
  void service.verify(r.id);
  return reply.code(202).send({ status: 'queued' });
});
app.get<{ Params: { id: string } }>('/api/uploads/:id/download', async (req, reply) => {
  const r = db.get(req.params.id);
  if (!r) return reply.code(404).send();
  if (!service.public(r).downloadable || !r.tusId)
    return reply.code(409).send({ error: 'Download is blocked until the required checks pass.' });
  try {
    ...
    reply
      ...
      .header(
        'Content-Disposition',
        `attachment; filename="download"; filename*=UTF-8''${encodeURIComponent(r.name)}`,
      )
      .header('Content-Length', length!)
      .header('X-Content-SHA256', r.actualSha256!);
...
await app.register(proxy, {
  ...
  preHandler: async (req, reply) => {
    if (!['HEAD', 'PATCH', 'OPTIONS'].includes(req.method))
      return reply
        .code(405)
        .send({ error: 'Use the application API to create or delete transfers.' });
    if (req.method === 'OPTIONS') return;
    let tusId: string;
    try {
      tusId = decodeURIComponent(req.url.split('/files/')[1]?.split('?')[0] || '');
    } catch {
    ...
    if (req.method === 'PATCH') {
      if (record.integrity !== 'pending')
        return reply.code(409).send({ error: 'Transfer already completed.' });
      db.patch(record.id, { updatedAt: Date.now() });
      service.active.set(record.id, (service.active.get(record.id) || 0) + 1);
      reply.raw.once('close', () => {
        const count = (service.active.get(record.id) || 1) - 1;
        if (count === 0) service.active.delete(record.id);
        else service.active.set(record.id, count);
      });
    }
  },
  replyOptions: {
    rewriteHeaders: (headers, req) => {
      const rawOffset = headers['upload-offset'];
      if (req && rawOffset !== undefined && ['HEAD', 'PATCH'].includes(req.method)) {
        const id = decodeURIComponent(req.url.split('/files/')[1]?.split('?')[0] || '');
        const record = db.all().find((r) => r.tusId === id);
        if (record) db.patch(record.id, { offset: Number(rawOffset) });
      }
      return headers;
    },
  },
...
interface Hook {
  Type: string;
  Event: { Upload: { ID: string; Size: number; Offset: number; MetaData: { recordId?: string } } };
}
hooks.post<{ Body: Hook }>('/hooks', async (req) => {
  const { Type, Event } = req.body;
  const upload = Event.Upload;
  const id = upload.MetaData?.recordId;
  const record = id ? db.get(id) : undefined;
  if (Type === 'pre-create')
    return !record || record.deleting || record.tusId || record.size !== upload.Size
      ? { RejectUpload: true }
      : {};
  if (record) {
    db.patch(record.id, { tusId: upload.ID });
    if (Type === 'post-finish' && !record.deleting) {
      db.patch(record.id, { offset: record.size });
      void service.verify(record.id);
    }
  }
  return {};
});
