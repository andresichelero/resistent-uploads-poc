...
export function validateInput(value: unknown): UploadInput {
  ...
  if (
    ...
    v.name.length > 200 ||
    /[\\/:]/.test(v.name) ||
    [...v.name].some((c) => c.charCodeAt(0) < 32 || c.charCodeAt(0) === 127)
  )
    throw new Error('Use a filename without paths or control characters (maximum 200 characters).');
  if (!Number.isSafeInteger(v.size) || v.size! < 0 || v.size! > MAX_SIZE)
    throw new Error('Files must be between 0 bytes and 100 MiB.');
  if (typeof v.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(v.sha256))
    throw new Error('A full SHA-256 digest is required.');
